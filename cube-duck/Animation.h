...
struct AnimationSequence {
	...
	Matrix interpolateBoneToGlobal(Matrix* matrices, int baseFrame, float interpolationFact, Skeleton* skeleton, int boneIndex) {
		Matrix scale = Matrix::setScaling(interpolate(frames[baseFrame].scales[boneIndex], frames[nextFrame(baseFrame)].scales[boneIndex], interpolationFact));
...
public:
	...
	void calcTransforms(Matrix* matrices, Matrix coordTransform) {
		for (int i = 0; i < bonesSize(); i++) {
			//matrices[i] = ((skeleton.bones[i].offset.mul(matrices[i])).mul(skeleton.globalInverse)).mul(coordTransform);
			matrices[i] = ((coordTransform.mul(skeleton.globalInverse)).mul(matrices[i])).mul(skeleton.bones[i].offset);
		}
	}
...
public:
	...
	bool animationFinished() {
		if (usingAnimation == "") {
			MessageBoxA(NULL, "No animation selected in AnimationInstance::animationFinished", "Error", MB_OK);
			return true;
		}

		if (!animation->hasAnimation(usingAnimation)) {
			MessageBoxA(NULL, ("Animation not found: " + usingAnimation).c_str(), "Error", MB_OK);
			return true;
		}
	...
	Matrix findWorldMatrix(std::string boneName) {
		...
		return (coordTransform.mul(matricesPose[boneID]));
	}
};